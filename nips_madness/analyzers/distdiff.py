...
def generated_tuning_curves(data, **kwargs):
    for J, D, S in data.iter_gen_params():
        yield sample_tuning_curves(J=J, D=D, S=S, **kwargs)


def calc_distdiff(data, **kwargs):
    solver_kwargs = dict(io_type=data.io_type, **kwargs)
    ...
    for fake, _ in generated_tuning_curves(data, **kwargs):
        print('.', end='')
...
def plot_distdiff(results):
    fig, ax = pyplot.subplots()
    for i, KSD in enumerate(results['KSD']):
        ax.plot(KSD, label=str(i))
    ax.set_xlabel('Epoch')
