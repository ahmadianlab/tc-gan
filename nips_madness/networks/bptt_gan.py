...
class EulerSSN(BaseComponent):
    ...
    def __init__(self, stimulator, J, D, S, k, n, tau_E, tau_I, dt,
        ...
        # J = np.broadcast_to(J, (2, 2))
        # D = np.broadcast_to(D, (2, 2))
        # S = np.broadcast_to(S, (2, 2))
        J = np.asarray(J)
        D = np.asarray(D)
        S = np.asarray(S)
        ...
        input_var = self.stimulator.stimulus * eps
        input_var = input_var.reshape((shape[0], 1, shape[2]))
        input_var = theano.tensor.tile(input_var, (1, shape[1], 1))
        self.l_inp = lasagne.layers.InputLayer(shape, input_var)

        self.zmat = theano.tensor.matrix('zmat')
        self.W = make_W_with_x(theano.tensor.shape_padleft(self.zmat),
                               self.J, self.D, self.S,
                               num_sites,
                               np.linspace(-0.5, 0.5, num_sites))[0].T

        I = theano.tensor.eye(num_neurons)
        self.l_rec = lasagne.layers.RecurrentLayer(
            self.l_inp, num_neurons,
            W_in_to_hid=I,
            W_hid_to_hid=I + (- I + self.W) / eps.reshape((-1, 1)),
            b=None,
            nonlinearity=ssnode.make_io_fun(k, n, io_type=io_type),
        )
        # TODO: this is actually not correct... Use CustomRecurrentLayer
        # http://lasagne.readthedocs.io/en/latest/modules/layers/recurrent.html#lasagne.layers.CustomRecurrentLayer
...
class TuningCurveGenerator(BaseComponent):
    ...
    def consume_kwargs(cls, **kwargs):
        ...
        model, rest = EulerSSN.consume_kwargs(stimulator, **rest)
