...
def test_dump_then_load(tmpdir):
    ...
    param_file.dump(l1, param_path)
    values = param_file.load(param_path)
...
def test_save_on_error(tmpdir):
    ...
    try:
        with param_file.save_on_error(l1, pre_path, post_path):
